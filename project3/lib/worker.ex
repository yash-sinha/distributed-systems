      ...
      receive do
          ...
          cond do
                    ...
                    cond do
                        (length(lessleaf) > 0 && toid >= Enum.min(lessleaf) && toid <= id)  || (length(largerleaf)> 0 && toid <= Enum.max(largerleaf) && toid >= id) ->
                             ...
                             if(abs(toid - id) > diff) do
                                IO.puts("cond00:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                       ...
                       (length(lessleaf) <= 4 && length(lessleaf) > 0 && toid < Enum.min(lessleaf)) ->
                             IO.puts("cond01:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                       ...
                       (length(largerleaf) <= 4 && length(largerleaf) > 0 && toid > Enum.max(largerleaf)) ->
                             IO.puts("cond02:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                       ...
                       getfromtable(table, samePre, String.to_integer(String.at(toBase4String(toid, log4), samePre)) != -1) ->
                             ...
                             IO.puts("cond03:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             messageworker(value, "route", [msg, fromid, toid, hops + 1, id])

                       (toid > id) ->
                             IO.puts("cond04:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             messageworker(Enum.max(largerleaf), "route", [msg,fromid, toid, hops + 1, id])
                             :global.whereis_name(:server) |> send({String.to_atom("notinboth")})

                       (toid < id) ->
                             IO.puts("cond05:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             messageworker(Enum.min(lessleaf), "route", [msg,fromid, toid, hops + 1, id])
                             :global.whereis_name(:server) |> send({String.to_atom("notinboth")})
              ...
              (msg == "route") ->
                    if(id == toid) do
                      ...
                      IO.puts("cond06finish:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                      ...
                      cond do
                         ((length(lessleaf) > 0 && toid >= Enum.min(lessleaf) && toid < id) ||
                            ...
                            if (abs(toid - id) > diff) do
                              ##IO.puts "cond1a"
                              IO.puts("cond07:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                              messageworker(nearest, "route", [msg, fromid, toid, hops + 1, id])
                            else
                              ...
                              IO.puts("cond08finish:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                          ...
                          (length(lessleaf) <= 4 && length(lessleaf) > 0 && toid < Enum.min(lessleaf)) ->
                            ##IO.puts "cond2"
                            IO.puts("cond09:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             messageworker(Enum.min(lessleaf), "route", [msg, fromid, toid, hops + 1, id])

                          (length(largerleaf) <= 4 && length(largerleaf) > 0 && toid > Enum.max(largerleaf)) ->
                            ##IO.puts "cond3"
                            IO.puts("cond10:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             messageworker(Enum.max(largerleaf), "route", [msg, fromid, toid, hops + 1, id])

                          ((length(lessleaf) == 0 && toid < id) || (length(largerleaf) == 0 && toid > id)) ->
                            ##IO.puts "cond4"
                            IO.puts("cond11finish:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             :global.whereis_name(:server) |> send({String.to_atom("routefinish"), [fromid, toid, hops]})


                          (toid > id) ->
                            ####IO.puts "cond6"
                            IO.puts("cond13:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                             messageworker(Enum.max(largerleaf), "route", [msg, fromid, toid, hops + 1, id])
                             :global.whereis_name(:server) |> send({String.to_atom("routenotinboth")})

                          (toid < id) ->
                            ####IO.puts "cond7"
                            IO.puts("cond14:: hops: #{hops} id #{id} from: #{fromid} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
                            messageworker(Enum.min(lessleaf), "route", [msg, fromid, toid, hops + 1, id])
                            :global.whereis_name(:server) |> send({String.to_atom("routenotinboth")})
        ...
        {:addleaf, [allleaf]} ->
          ...
          {:clocktick} ->
            ...
            IO.puts("cond15:: hops: -1 id #{id} from: #{id} toid: #{toid} lessleaf:  #{inspect(lessleaf)} : largerleaf: #{inspect(largerleaf)} : table is: #{inspect(table)} ")
